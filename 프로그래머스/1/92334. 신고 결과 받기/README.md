...
## 리팩토링하며 배운 점
동일한 신고를 한 번만 처리해야 하므로, 피신고자별 신고자 목록을 리스트가 아닌 집합에 담았다.  
`set.add()`는 중복 원소를 자동으로 제거하므로 별도의 포함 여부 검사 없이 효율적으로 신고자를 추가할 수 있다.

---

### 성능 요약

메모리: 36.2 MB, 시간: 267.65 ms
